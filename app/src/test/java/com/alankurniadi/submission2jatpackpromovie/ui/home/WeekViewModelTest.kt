...
class WeekViewModelTest {
    ...
    @Mock
    private lateinit var observer: Observer<List<TrendingWeek.DataWeek>>

    @Before
    fun setUp() {
        viewModel = WeekViewModel(movieDbRepository)
    }

    @Test
    fun getWeekTrending() {
        val trendingList = MutableLiveData<List<TrendingWeek.DataWeek>>()
